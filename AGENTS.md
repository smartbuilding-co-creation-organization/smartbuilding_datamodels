...
- **Required tooling**: Use Python 3.13.1+ and the latest Git for any local validation or generation tasks.
- **Editor extensions**: Follow `.vscode/extensions.json` recommendations (JSON/YAML tooling and Prettier) for consistent formatting and linting.
- **GitHub Flow**: `main` is the single source of truth; create `feature/...` or `fix/...` branches and open PRs back to `main`.
- **Samples required**: When adding or changing a data model that expects examples, provide matching sample data under `examples/` alongside the schema updates.
- **Validation**: Run `npm run validate` for JSON Schema contributions before opening a PR.
- **Naming rules**: Use `camelCase` for field names, `kebab-case.json` for schema filenames, and consistent enum casing (`SCREAMING_SNAKE_CASE` or `camelCase`).
- **JSON Schema rules**: Every schema must define a unique `$id`, include Japanese descriptions for all fields, and apply strict constraints (`minimum`, `maximum`, `pattern`, `minLength`, `minItems`, `uniqueItems`) where applicable.
- **Reuse shared definitions**: Prefer `$ref` to shared items under `schemas/common/` when working with JSON Schema assets.
- **Backward compatibility**: Avoid removing fields, changing types, or adding required constraints without explicit breaking-change notes.
- **Commit format**: Use Conventional Commits (`feat`, `fix`, `docs`, `refactor`) and include `BREAKING CHANGE:` notes when needed.
- **PR checklist**: Confirm validation passes, examples are present, descriptions are clear, and the change is non-breaking (or marked as breaking).
- **Release expectations**: Merges to `main` trigger automated versioning, tagging, and release notes; keep PRs aligned with that flow.
